...
class TestTriangle:
    ...
    def test_area(self, trin_obj):
        assert trin_obj.get_triangle_area() == 78.0, f'Некорректное значение площади фигуры, ответ системы: {trin_obj.get_area()}'
    ...
    def test_add_square(self, trin_obj, quad_obj):
        ...
        assert sum_area == 222.0, f'Некорректный ответ, ожидалось сумма площадей = 222.0, ответ = {sum_area}'
...
class TestRectangle:
    ...
    def test_area(self, rect_obj):
        assert rect_obj.get_rectangle_area() == 156, f'Некорректное значение площади фигуры, ответ системы: {rect_obj.get_area()}'
    ...
    def test_add_square(self, rect_obj, quad_obj):
        ...
        assert sum_area == 300, f'Некорректный ответ, ожидалось сумма площадей = 300, ответ = {sum_area}'
...
class TestQuadrate:
    ...
    def test_area(self, quad_obj):
        assert quad_obj.get_quadrate_area() == 144, f'Некорректное значение площади фигуры, ответ системы: {quad_obj.get_area()}'
...
class TestCircle:
    ...
    def test_area(self, circl_obj):
        assert int(circl_obj.get_circle_area()) == 113, f'Некорректное значение площади фигуры, ответ системы: {circl_obj.get_area()}'
    ...
    def test_add_square(self, circl_obj, quad_obj):
        ...
        assert sum_area == 257, f'Некорректный ответ, ожидалось сумма площадей = 257, ответ = {sum_area}'
